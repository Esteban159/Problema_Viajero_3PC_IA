...
def main():
  ...
  for i in range(0, 100):
    ...
    if i==0:
      fittestDistanceAnt=pop.getFittest().getDistance()
    else:
      fittestDistance=pop.getFittest().getDistance()
      GainByGeneration.append(fittestDistanceAnt-fittestDistance)
      posicion.append(i+1)
      fittestDistanceAnt=fittestDistance
    print i
  plt.bar(posicion,GainByGeneration)
  plt.xlabel('Numero de generacion')
  plt.ylabel('Ganancia de Distancias respecto al anterior')
  plt.title('Ganancias de Distancias de todas las Generaciones')
  plt.show()
